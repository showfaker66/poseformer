...
def parse_args():
    ...
    # parser.add_argument('-cs', '--checkpoints', default='checkpoints', type=str, metavar='PATH',
    #                     help='checkpoint directory')
    # parser.add_argument('-c', '--checkpoint', default='/kaggle/working', type=str, metavar='PATH',
    #                     help='checkpoint directory')
    parser.add_argument('-c', '--checkpoint', default='checkpoint', type=str, metavar='PATH',
                        help='checkpoint directory')
    parser.add_argument('--checkpoint-frequency', default=40, type=int, metavar='N',
                        help='create a checkpoint every N epochs')
    # parser.add_argument('-r', '--resume', default='../checkpoint/pretrained_h36m_cpn.bin', type=str, metavar='FILENAME',  # 从一个特定的 checpoint 重新开始训练
    #                     help='checkpoint to resume (file name)')
    parser.add_argument('-r', '--resume', default='', type=str, metavar='FILENAME',
                        # 从一个特定的 checpoint 重新开始训练
                        help='checkpoint to resume (file name)')
    ...
    parser.add_argument('--export-training-curves', default= 1, action='store_true', help='save training curves as .png images')


    # Model arguments
    parser.add_argument('-s', '--stride', default=1, type=int, metavar='N', help='chunk size to use during training')
    parser.add_argument('-e', '--epochs', default=30, type=int, metavar='N', help='number of training epochs')
    parser.add_argument('-b', '--batch-size', default=1280, type=int, metavar='N', help='batch size in terms of predicted frames')
    parser.add_argument('-drop', '--dropout', default=0.25, type=float, metavar='P', help='dropout probability')
    parser.add_argument('-lr', '--learning-rate', default=0.001, type=float, metavar='LR', help='initial learning rate')
    parser.add_argument('-lrd', '--lr-decay', default=0.95, type=float, metavar='LR', help='learning rate decay after per epoch')
    ...
    parser.add_argument('--dense', action='store_true', help='use dense convolutions instead of dilated convolutions，This is only useful for benchmarks and ablation experiments.')
    parser.add_argument('--disable-optimizations', action='store_true', help='disable optimized model for single-frame predictions') # 且当stride = 1时.This is only useful for benchmarks.
    ...
    # Visualization
    parser.add_argument('--viz-subject', type=str, metavar='STR', help='subject to render')  # S11
    parser.add_argument('--viz-action', type=str,  metavar='STR', help='action to render')    # Waliking
    ...
    parser.add_argument('--viz-output', type=str, metavar='PATH', help='output file name (.gif or .mp4)')  # output.gif
    ...
    parser.add_argument('--viz-limit', type=int, metavar='N', help='only render first N frames')  # 60
    parser.add_argument('--viz-downsample', type=int, metavar='N', help='downsample FPS by a factor N')  # 2
    parser.add_argument('--viz-size', type=int, metavar='N', help='image size')  # 3
