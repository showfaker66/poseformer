...
cfg_dir = r'I:\Human_pose_estimation\MHFormer-main\demo\lib\hrnet\experiments\\'
# model_dir = '../../../demo/lib/checkpoint/'
model_dir = r'I:\Human_pose_estimation\MHFormer-main\demo\lib\checkpoint\\'
...
def gen_video_kpts(video, det_dim=416, num_peroson=1, gen_output=False):
    ...
    video_length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))  # 291帧
    ...
    for ii in tqdm(range(video_length)):
        ...
        if not ret:
            continue
        # 预测框2个对角的坐标bboxs: 584.3760, 104.8172, 761.8487, 694.0070   scores: 0.9999
        bboxs, scores = yolo_det(frame, human_model, reso=det_dim, confidence=args.thred_score)  # 置信度 0.3
        ...
        if bboxs is None or not bboxs.any():
            ...
            bboxs_pre = copy.deepcopy(bboxs)  # 1, 4
            scores_pre = copy.deepcopy(scores)  # 0.9998

        # Using Sort to track people
        people_track = people_sort.update(bboxs) # (1, 5)  584.38  104.82  761.84   694.01   1

        # Track the first two people in the video and remove the ID
        if people_track.shape[0] == 1:
            people_track_ = people_track[-1, :-1].reshape(1, 4)   # [-1, :-1] 除了最后一列取全部
        elif people_track.shape[0] >= 2:
            people_track_ = people_track[-num_peroson:, :-1].reshape(num_peroson, 4)
            people_track_ = people_track_[::-1]
        else:
            continue
        #  people_track_: 584.38  104.82  761.84   694.01
        track_bboxs = []
        ...
        with torch.no_grad():
            # bbox is coordinate location 前处理 原视频帧(720, 1280, 3)经过仿射变换得到高分辨率net的输入inputs
            # inputs: (1, 3, 384, 288)   origin_img: (720, 1280, 3)  center: (673.115, 399.415)  scale: (2.0713, 3.6824)
            inputs, origin_img, center, scale = PreProcess(frame, track_bboxs, cfg, num_peroson)

            inputs = inputs[:, [2, 1, 0]]  # 1, 3, 384, 288

            if torch.cuda.is_available():
                inputs = inputs.cuda()
            output = pose_model(inputs)  # 1, 17, 96, 72
        ...
        for i, score in enumerate(maxvals):
            scores[i] = score.squeeze()   # score: 17, 1 -> 1, 17  列->行

        kpts_result.append(kpts)
        scores_result.append(scores)

    keypoints = np.array(kpts_result)  # 291, 1, 17, 2  291帧 每帧人体关键点坐标
    scores = np.array(scores_result)   # 291, 1, 17     291帧 每帧人体关键点分数

    keypoints = keypoints.transpose(1, 0, 2, 3)  # (291, 1, 17, 2) -> (1, 297, 17, 2)
    scores = scores.transpose(1, 0, 2)  # (291, 1, 17) -> (1, 291, 17)
