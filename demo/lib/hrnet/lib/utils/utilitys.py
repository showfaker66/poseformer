...
# Pre-process
def PreProcess(image, bboxs, cfg, num_pos=1):
    if type(image) == str:
    ...
    for bbox in bboxs[:num_pos]:
        c, s = box_to_center_scale(bbox, data_numpy.shape[0], data_numpy.shape[1])  # c:预测框中心的坐标 (673.115, 399.415)  s: 2.0713, 3.6824
        centers.append(c)
        scales.append(s)
        r = 0

        trans = get_affine_transform(c, s, r, cfg.MODEL.IMAGE_SIZE)  # 2, 3
        input = cv2.warpAffine(  # data_numpy 经过仿射变换矩阵trans 按照IMAGE_SIZE尺寸进行变换   384, 288, 3
            data_numpy,
            trans,
        ...
        input = transform(input).unsqueeze(0)  # 1, 3, 384, 288
        inputs.append(input)
