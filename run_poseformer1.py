...
# keypoints_metadata['layout_name'] = args.dataset
...
# #group_size=int(pow(17,0.5))
# model_pos = PoseTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=32, depth=4,
#         num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_rate=0.,attn_drop_rate=0.,drop_path_rate=0.2)

model_pos_train = PoseTransformer(num_frame=receptive_field, num_joints=num_joints, drop_rate=0.,norm_layer=None)
#group_size=int(pow(17,0.5))
model_pos = PoseTransformer(num_frame=receptive_field, num_joints=num_joints,drop_rate=0.,norm_layer=None)
...
if args.resume or args.evaluate:
    ...
    # chk_filename = os.path.join(args.checkpoints, args.resume if args.resume else args.evaluate)
...
if not args.evaluate:
    ...
    lr_decay_large = args.lr_decay_large
    large_decay_epoch = args.large_decay_epoch
    losses_3d_train = []
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            cameras_train = torch.from_numpy(cameras_train.astype('float32'))  # 16,9
            inputs_3d = torch.from_numpy(batch_3d.astype('float32'))  # 16,1,17,3
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))  # 16,27,17,2
            ...
            # predicted_3d_pos, pos_ST = model_pos_train(inputs_2d)
            # 16,1,17,3       16,27,544
            del inputs_2d
            torch.cuda.empty_cache()

            # loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d) + mpjpe(pos_ST, inputs_3d)
            ...
            optimizer.step()
            # writer.add_scalar('Train_loss',loss_total)
        ...
        if epoch % large_decay_epoch == 0:
            for param_group in optimizer.param_groups:
                param_group['lr'] *= lr_decay_large
                lr *= lr_decay_large
        else:
            for param_group in optimizer.param_groups:
        ...
        if args.export_training_curves and epoch > 3:
            ...
            plt.plot(epoch_x, losses_3d_train[3:], '--', color='C0')
            plt.plot(epoch_x, losses_3d_train_eval[3:], color='C0')
            plt.legend(['3d train', '3d train (eval)'])
            plt.ylabel('MPJPE (m)')
            plt.xlabel('Epoch')
            plt.xlim((3, epoch))
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            ##### convert size
            inputs_2d, inputs_3d = eval_data_prepare(receptive_field, inputs_2d, inputs_3d)  # 1621，81, 17, 2   1621, 1, 17, 3
            inputs_2d_flip, _ = eval_data_prepare(receptive_field, inputs_2d_flip, inputs_3d)
...
if args.render:
    ...
    if args.viz_subject in dataset.subjects() and args.viz_action in dataset[args.viz_subject]:
        if 'positions_3d' in dataset[args.viz_subject][args.viz_action]:
            ground_truth = dataset[args.viz_subject][args.viz_action]['positions_3d'][args.viz_camera].copy()  # 1621, 17, 3
    ...
    # prediction = torch.randn(1621, 1, 17, 3)
...
#             ground_truth = ground_truth.permute(1, 0, 2, 3)
#             ground_truth = ground_truth.numpy()
            trajectory = ground_truth[:, :1]  # 1621, 1, 17, 3
            ground_truth[:, 1:] += trajectory # ground_truth[:, 1:] 1621, 16, 3
        ...
        anim_output = {'Ours(Pooling)': prediction}
